...
def collectFileList(file_path):
    ...
    print
    u"获取文件列表结束，用时: " + str(end - start) + u"秒"
...
def getWordsHashDictionary(file_name_list, k):
    ...
    pro.finish()
    print
    u"读取文件,处理词汇列表结束，用时: " + str(end - start) + u"秒"
...
def calcMinHashTableList(hash_table, min_hash_args):
    ...
    pro.finish()
    print
    u"计算签名哈希列表结束，用时: " + str(end - start) + u"秒"
...
def createSignaturesMatrix(union_table, sig_hash_len, hash_table):
    ...
    pro.finish()
    print
    u"生成签名矩阵结束，用时: " + str(end - start) + u"秒"
    return sig_mat_list
...
def calcEachSimilarity(sig_mat_list, file_names):
    ...
    print
    u"计算相似度结束，用时: " + str(end - start) + u"秒"
